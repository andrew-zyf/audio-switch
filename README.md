...
macOS 音频设备自动切换工具 — DJI Mic 输入 / AirPods 输出优先。
...
- 设备通过名称正则匹配（`airpods` / `dji.*mic`），不硬编码型号
...
脚本通过 [SwitchAudioSource](https://github.com/deweller/switchaudio-osx) 列出当前可用音频设备（纯文本，每行一个设备名），按正则模糊匹配识别 DJI Mic 和 AirPods，然后根据规则表执行切换。回退设备通过名称模式匹配内建硬件（如 `MacBook Pro麦克风`、`MacBook Pro扬声器`），避免误选显示器的 HDMI/DP 音频。
